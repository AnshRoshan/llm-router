...
import { affinityPrior, pickBackend, SessionPolicy, SessionStore, } from "./affinity.js";
import { chooseTtlBySegment, evaluateSwitch, } from "./economics.js";
import { extractFeatures } from "./learning.js";
import { GAP_PRIOR, TURNS_PRIOR, WorkloadResolver, } from "./signals.js";
import { Decision, SegmentKind, WorkloadClass, } from "./types.js";
...
export class PolicyEngine {
    ...
    qualityModel = null;
    constructor(prices, backends, resolver = new WorkloadResolver(), config = DEFAULT_CONFIG, sessions = null, qualityModel = null) {
        this.prices = prices;
    ...
    quality(card, workload, features) {
    ...
    estimateTurnCost(card, prefixTokens, hitRate, outputTokens = 500) {
        const cached = Math.trunc(prefixTokens * hitRate);
        const uncached = prefixTokens - cached;
        const sticker = card.cacheReadCost(cached) + card.inputCost(uncached) + card.outputCost(outputTokens);
        return sticker * card.taskCostMultiplier;
    }
    // ------------------------------------------------------------------ //
    // the decision
    // ------------------------------------------------------------------ //
    decide(req, now = nowMonotonic()) {
        const reasons = [];
        ...
        if (this.qualityModel && features) {
            ...
            if (covered > 0) {
                reasons.push(`learned-quality: ${covered}/${total} models have trained heads ` +
        ...
        if (session) {
            if (session.invalidatorsHash === null) {
                ...
                session.invalidatorsHash = invFp;
                reasons.push("Rule F: cache-invalidating request fields changed (tool " +
        ...
        for (const card of this.prices.all()) {
            if (!allowed.has(card.modelId))
                continue;
            if (!this.passesHardConstraints(card, req, workload, totalTokens, features))
                continue;
            for (const backend of this.backends.availableFor(card.modelId, now)) {
                ...
                const latency = (health.ewmaLatencyMs || card.p50LatencyMs) +
                    health.inFlight * 5.0 +
                    backend.coldStartRisk * 2000.0;
                const isWarm = !!session &&
                ...
                if (remaining !== null && !overCap) {
                    ...
                    if (projected > remaining)
                        budgetPenalty = projected - remaining;
                }
                const candReasons = [`hit_rate=${hitRate.toFixed(2)}`, `warm=${isWarm}`];
                if (budgetPenalty > 0) {
                    candReasons.push(`budget pacing: projected $${projected.toFixed(4)} exceeds ` +
                        `$${remaining.toFixed(4)} remaining`);
                }
                const score = w.quality * quality -
        ...
        if (scored.length === 0 && overBudget.length > 0) {
            ...
            budgetFallback = true;
            const cheapest = scored.reduce((a, b) => a.candidate.costUsd <= b.candidate.costUsd ? a : b);
            reasons.push(`budget cap $${cap.toFixed(4)} excludes every candidate; ` +
                `chose the cheapest (${cheapest.candidate.modelId})`);
        }
        if (scored.length === 0) {
            throw new Error(`every candidate was filtered out for workload=${workload}; ` +
        ...
        if (session) {
            const pinnedAvailable = this.backends.availableFor(session.pinnedModel, now).length > 0;
            ...
            if (wantsSwitch && !pinnedAvailable) {
                ...
                reasons.push(`forced failover: pinned ${session.pinnedModel} has no healthy ` +
            ...
            if (wantsSwitch && !forced) {
                ...
                const tgt = best.card;
                if (cur && tgt) {
                    const verdict = evaluateSwitch(cur, tgt, prefixTokens, turnsRemaining, session.alreadySwitched);
        ...
        if (sem.supportsMixedTtl &&
            [...ttlBySegment.values()].includes("1h")) {
            reasons.push(`mixed TTL: ${[...ttlBySegment.entries()].map(([k, v]) => `${k}=${v}`).join(", ")}`);
        }
        // ---- 7. build the decision ----------------------------------------------
        const ordered = scored.map((s) => s.candidate);
        let final = ordered.find((c) => c.modelId === chosenModel && c.backendId === chosenBackend) ?? null;
        if (!final) {
            ...
            if (!final) {
                ...
                if (session && session.pinnedModel !== chosenModel) {
                    switched = true;
                    session.switchedAt = now;
                    reasons.push(`pinned ${session.pinnedModel} produced no viable candidate; ` +
                        `fell back to ${chosenModel}`);
                }
            }
            chosenBackend = final.backendId;
        }
        return new Decision(final.modelId, final.backendId, workload, signal.source, signal.confidence, stickyKey, stickyKey, req.isFirstTurn, session !== null, switched, stranded, ttlBySegment, ordered, reasons);
    }
    /**
     ...
     */
    recordDecision(req, d, usage, now = nowMonotonic()) {
        let session = this.sessions.get(d.stickyKey, now);
        if (!session) {
            session = new SessionPolicy(d.stickyKey, d.workload, d.workloadSource, d.workloadConfidence, d.modelId, d.backendId, now);
    ...
    allowedModels(req) {
        ...
        if (req.agentProfile) {
            ...
            if (profile?.allowedModels && profile.allowedModels.length > 0) {
                allowed = new Set(profile.allowedModels.filter((m) => this.prices.get(m) !== undefined));
            }
        }
        const pool = req.constraints.allowedModels;
        if (pool && (pool instanceof Set ? pool.size > 0 : [...pool].length > 0)) {
            const restricted = new Set([...pool].filter((m) => this.prices.get(m) !== undefined));
            allowed = allowed === null ? restricted : new Set([...allowed].filter((m) => restricted.has(m)));
        }
        return allowed ?? new Set(this.prices.ids());
    }
    passesHardConstraints(card, req, workload, promptTokens, features = null) {
        const quality = this.quality(card, workload, features);
        if (quality < 0)
            return false;
        ...
        if (req.constraints.requireTools && !card.supportsTools)
            return false;
        if (!card.supportsVision && req.hasImages())
            return false;
        const tags = req.constraints.residencyTags;
        if (tags && tags.size > 0) {
            for (const t of tags) {
                if (!card.residencyTags.has(t))
                    return false;
            }
        }
        // Pre-call context validation (full prompt + headroom for the response).
        if (card.contextWindow &&
            promptTokens + this.config.contextHeadroomTokens > card.contextWindow) {
            return false;
        }
        return true;
    }
    segmentTokens(req, prefixTokens) {
        const toks = (s) => Math.trunc(s.length * this.config.estTokensPerChar);
        ...
        const out = new Map();
        if (toolsT)
            out.set(SegmentKind.Tools, toolsT);
        if (sysT)
            out.set(SegmentKind.System, sysT);
        if (retrT)
            out.set(SegmentKind.Retrieved, retrT);
        if (histT)
            out.set(SegmentKind.History, histT);
        return out;
    }
}
